...
FROM f_comment JOIN f_user ON f_comment.profile_id=f_user.profile_id
WHERE user_id=4001;
...
FROM 
	...
	FROM uses_application JOIN application ON uses_application.application_id=application.application_id
...
FROM
	...
	FROM album JOIN f_page ON album.profile_id=f_page.profile_id
...
FROM message
	JOIN f_user AS gonderici ON message.sender_profile_id=gonderici.profile_id
	JOIN f_user AS alici ON message.receiver_profile_id=alici.profile_id;

#Query-8
SELECT a.page_id
FROM
	(SELECT page_id,manages.community_id AS com_id
	FROM manages JOIN f_page ON f_page.profile_id=manages.profile_id) AS a
	JOIN attends ON (SELECT event_id FROM f_event WHERE community_id=a.com_id)=attends.event_id
...
FROM
	...
	FROM attends JOIN f_event ON attends.event_id=f_event.event_id
	JOIN location ON f_event.location_id=location.location_id
